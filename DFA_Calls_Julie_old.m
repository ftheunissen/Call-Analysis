...
for inb = 1:length(PCC_info)
    ...
    PCC_Total = 100.0*sum(diag(ConfMat_DFA))./(n_valid*n_boot);
...
PCC_info_bird = struct('nb', {10, 20, 30, 40, 50, 75, 100, 150, 200, 250, 300}, 'nvalid', 0, ...
...
for inb = 1:length(PCC_info_bird)
    ...
    for iperm=1:nPerm
        ...
        B = TreeBagger(300, X_fit, Group_fit, 'FBoot', 1.0, 'OOBPred', 'on', 'MinLeaf', 5, 'NPrint', 10);        
    ...
    PCC_Total_DFA = 100.0*sum(diag(ConfMat_DFA))./n_validTot;
    PCC_Total_RF = 100.0*sum(diag(ConfMat_RF))./n_validTot;
    PCC_group_DFA = zeros(ngroups, ngroups);
    PCC_group_RF = zeros(ngroups, ngroups);
    for i = 1:ngroups
        for j = 1:ngroups
            PCC_group_DFA(i,j) = ConfMat_DFA(i,j) / sum(ConfMat_DFA(i, :), 2) * 100;
            PCC_group_RF(i,j) = ConfMat_RF(i,j) / sum(ConfMat_RF(i, :), 2) * 100;
        end
    end
    PCC_M_DFA = mean(diag(PCC_group_DFA));
    PCC_M_RF = mean(diag(PCC_group_RF));
    
    % Store the information
    PCC_info_bird(inb).nvalid = n_validTot;
    
    PCC_info_bird(inb).PCC_Total_DFA = PCC_Total_DFA;
    PCC_info_bird(inb).PCC_M_DFA = PCC_M_DFA;
    PCC_info_bird(inb).PCC_group_DFA = PCC_group_DFA;
    
    PCC_info_bird(inb).PCC_Total_RF = PCC_Total_RF;
    PCC_info_bird(inb).PCC_M_RF = PCC_M_RF;
    PCC_info_bird(inb).PCC_group_RF = PCC_group_RF;
...
for inb = 1:length(PCC_info_bird)
    ...
    subplot(2,length(PCC_info_bird),inb);
    imagesc(PCC_info_bird(inb).PCC_group_DFA);
    xlabel('Guess');
    ylabel('Actual');
    colormap(gray);
    colorbar;
    title(sprintf('Confusion Matrix DFA %.1f%%(%.1f%%) Correct', PCC_info_bird(inb).PCC_Total_DFA, PCC_info_bird(inb).PCC_M_DFA));
    set(gca(), 'Ytick', 1:ngroups);
    set(gca(), 'YTickLabel', name_grp);
    set(gca(), 'Xtick', 1:ngroups);
    set(gca(), 'XTickLabel', name_grp);
    
    subplot(2,length(PCC_info_bird),length(PCC_info_bird)+inb);
    imagesc(PCC_info_bird(inb).PCC_group_RF);
    xlabel('Guess');
    ylabel('Actual');
    colormap(gray);
    colorbar;
    title(sprintf('Confusion Matrix RF %.1f%%(%.1f%%) Correct', PCC_info_bird(inb).PCC_Total_RF, PCC_info_bird(inb).PCC_M_RF));
    set(gca(), 'Ytick', 1:ngroups);
    set(gca(), 'YTickLabel', name_grp);
    set(gca(), 'Xtick', 1:ngroups);
    set(gca(), 'XTickLabel', name_grp);
...
figure(9);
plot([PCC_info_bird.nb], [PCC_info_bird.PCC_Total_DFA],'k');
hold on;
plot([PCC_info_bird.nb], [PCC_info_bird.PCC_M_DFA],'r');
plot([PCC_info_bird.nb], [PCC_info_bird.PCC_Total_RF],'k--');
plot([PCC_info_bird.nb], [PCC_info_bird.PCC_M_RF],'r--');
xlabel('Number of PCs');
ylabel('Correct Classification');
...
% order
tosortMatrix = PCC_info_bird(inb).PCC_group;
...
colormap(gray);
colorbar;
title(sprintf('Spectro Confusion Matrix DFA\nnPCA = %d PC Total= %.1f%% PCC Mean = %.1f%%', PCC_info_bird(inb).nb, PCC_info_bird(inb).PCC_Total, PCC_info_bird(inb).PCC_M));
set(gca(), 'Ytick', 1:ngroups);
