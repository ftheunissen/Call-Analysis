...
outputTable = '/Users/frederictheunissen/Documents/Data/Julie/Acoustical Analysis/pairCallerCrossDiscrimSpectroResults.h5'
outputGroupedTable = '/Users/frederictheunissen/Documents/Data/Julie/Acoustical Analysis/pairCallerCrossDiscrimSpectroGroupedResults.h5'
fileExcelTable = '/Users/frederictheunissen/Documents/Data/Julie/Acoustical Analysis/pairCallerCrossDiscrimSpectroResults.xls'
...
for gp in goodPairs:    
    ...
    for ctypeTest in callTypeUnique:
        ...
        for ctypeTrain in callTypeUnique:
            ...
            if countType1 < MINCOUNT or countType2 < MINCOUNT:
                continue
            trainIndAll = np.asarray(callTypezPair == ctypeTrain).nonzero()[0]
            trainIndType = np.random.choice(trainIndAll, size = gp['minCalls'], replace = False )
            trainInd = np.hstack((trainInd, trainIndType))
        ...
        for fl in featureList:
            ...
            ldaY, qdaY, rfY, cvC, ldaP, qdaP, rfP, nC, ldaweights = discriminatePlot(fl['X'][testInd], birdzPair[testInd], cValBirdPair[testInd], 
                                                                                 titleStr='Caller %s (%s vs %s) Spectro' % (ctypeTrain, gp['bird1'], gp['bird2']), 
                                                                                 figdir = figdir)
...
indCallerAll = [index for index, row in resultsAgg.iterrows() if ( (row.SexPair == "A") and (row.Features == 'Spectro') and (row.TestType == 'Caller'))]
...
for lbl in sortedCallTypes:
    xvals += 1
    indVoiceAll += [index for index, row in resultsAgg.iterrows() if ( (row.SexPair == 'A') and (row.Call == lbl) and (row.Features == 'Spectro') and (row.TestType == 'Voice')) ]
...
# Select rows for all sex combinations and all Features.
indCallerAll = [index for index, row in resultsDataFrame.iterrows() if ((row.Features == 'Spectro') and (row.TestType == 'Caller'))]
